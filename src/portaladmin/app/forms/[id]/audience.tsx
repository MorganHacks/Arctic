...
export function Audience({
  ...
  if (kind === "application") {
    return (
      <section className={styles.settingsPanel} data-setting="audience" aria-label="Who can answer">
        <div className={`${styles.settingsSummary} ${styles.settingsStatic}`}>
          <span className={styles.settingsIcon}><Icon icon={UserGroupIcon} size={19} /></span>
          <span className={styles.settingsCopy}>
            <span>Who can answer</span><strong>Anyone with the link</strong>
            <small>No sign-in needed to apply.</small>
          </span>
        </div>
      </section>
      );
    ...
    return (
      <details className={styles.settingsPanel} data-setting="audience">
        <summary className={styles.settingsSummary}>
          <span className={styles.settingsIcon}><Icon icon={UserGroupIcon} size={19} /></span>
          <span className={styles.settingsCopy}><span>Who can answer</span><strong>{gated ? "Selected applicants" : "Anyone with the link"}</strong></span>
        ...
        <p className="meta">
          Applicants receive a sign-in link by email. Responses are linked to their account.
        </p>

        {gated ? (
          <>
            <p className="meta">Allow applicants with these statuses:</p>
        ...
        {canManage ? (
          <div className={styles.settingsActions}>
            <button
              ...
              onClick={() => void save()}
            >
              {saving ? "Saving…" : saved ? "Saved" : "Save audience"}
            </button>
          </div>
        ) : null}
